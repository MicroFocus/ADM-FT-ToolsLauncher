...
| `almTestSetsOrderByCriteria` | string | `id` or `name` | (*Optional*) The criteria used to order the execution of test sets. Use `id` to sort by test set ID in ascending order, or `name` to sort alphabetically by test set name. Left empty, the test sets will be executed alphabetically by their name. |
