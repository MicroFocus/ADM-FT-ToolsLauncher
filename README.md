...
| `resultsFileLangTag` | string | `Default`<br/>-or-<br/>`System`<br/>-or-<br/>&lt;language-tag&gt; | (*Optional*) The language tag used to generate the localization content in the test results summary file. The localization content includes the number, data and time formats. For example, the number `12.34` is generated as is in English (language tag `en-US`) while it is `12,34` in German (language tag `de-DE`). The value can be either a valid language tag or one of the case-insensitive constants.<br/><br/>If the value is `Default` or it is not set, the content is always generated in the neutral English language.<br/><br/>If the value is `System`, the application will automatically detect the language used in the system and use that language to do localization. In most cases, the language tag is same as the display language configured in the system.<br/><br/>The value can also be one of the valid case-insensitive language tag names such as `en-US`, `de-DE` and so on. For a list of predefined language tag names on Windows systems, see the **Language tag** column in the [list of language/region names supported by Windows][msdoc-list-of-langauge-region-names-supported-by-windows]. The names follow the standard defined by [BCP 47][bcp47-url]. In addition, starting with **Windows 10**, name can be any valid BCP-47 language tag. |
