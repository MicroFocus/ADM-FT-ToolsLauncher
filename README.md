...
| `resultTestNameOnly` | boolean | `true` _or_ *`false`* | (*Optional*) Indicates whether the test name shall be populated instead of the full path of the **UFT One** tests in the test results summary file. The **LoadRunner** tests are not affected. Default = `false`.<br/><br/>For example, by default the full path of the **UFT One** test is populated in the test results summary file like `<testcase name="C:\tests\GUITest1"`, and if this parameter is set to `true` then only the test name will be populated like `<testcase name="GUITest1"`. |
