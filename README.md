...
| `unstableAsFailure` | boolean | `true` _or_ *`false`* | **Introduced in `v1.0.27.3331` (`v1.0-beta-rev9`)**.<br/><br/>(*Optional*) Indicates whether this launcher tool shall treat the **Unstable** test result as **Failure**. The exit code is affected if the test result is **Unstable**. See more details in [Exit Code](#fttools-exit-code) section. |  
...
Starting from `v1.0.27.3331` (`v1.0-beta-rev9`), the **FTToolsLauncher** tool can report the correct exit code when the process exits. The **FTToolsLauncher** tool may return one of the following exit codes:
